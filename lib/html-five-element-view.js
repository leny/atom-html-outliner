...
export default class HTMLFiveElementView extends View {
    ...
    constructor( oSection, iIndex = 0, bIsRoot = false ) {
        ...
        if ( oSection.heading.implied ) {
            ...
            this.titleValue.text( "Untitled (no head element)" );
        } else {
        ...
        if ( oSection.sections && oSection.sections.length > 0 ) {
            ...
            oSection.sections.forEach( ( oSubSection, iSubIndex ) => {
                this.subsectionsContainer.append( new HTMLFiveElementView( oSubSection, iSubIndex ) );
            } );
        } else {
